...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
        ...
        for symbol in symbols {
                ...
                (symbol["symbol"].as_str(), symbol["quoteAsset"].as_str())
            {
                if quote_asset == "BTC" {
                    ...
                    coins.push(format!("BINANCE:{}BTC", base_asset));
                }
            }
        }
    }
    ...
    if coins.is_empty() {
        println!("BTC spot piyasa çiftleri bulunamadı. API yanıtını kontrol edin.");
        return Ok(());
    }
    ...
    let mut file = File::create("binance_btc_markets.txt")?;
    for coin in &coins {
        writeln!(file, "{}", coin)?;
    }

    println!("Binance BTC spot piyasası coin listesi başarıyla oluşturuldu ve kaydedildi.");
    println!("Toplam {} adet BTC spot piyasa çifti bulundu.", coins.len());
    Ok(())
}
